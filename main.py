...
print("\n=== FIRE DETECTION SYSTEM – REALTIME ML MODE ===\n")
...
try:
    while True:
        ...
        relay_active = temporal_result["relay_active"]
        ...
        ml_record = {
            ...
            "temp_c": float(temp_c),
            "mq2_hi": float(mq2_hi),
            "mq135_hi": float(mq135_hi),
            "voc_ppm": int(voc_ppm),
            "ml_hazard_label": int(ml_label)
        }
        ...
        # ---------- ERA PAYLOAD ----------
        era_payload = {
            "system_status": temporal_result["confirmed_hazard"],
            ...
            "temporal_hazard": temporal_result["confirmed_hazard"],
            "streak": temporal_result["streak"],
            "required_count": temporal_result["required_count"],

            "relay_active": temporal_result["relay_active"],
            "buzzer": temporal_result["actuator"]["buzzer"],
            "fan": temporal_result["actuator"]["fan"],
            "mist": temporal_result["actuator"]["mist"],
            "emergency": temporal_result["actuator"]["emergency"]
        }
        ...
        print(f" 🔌 RELAY      → {'ON' if relay_active else 'OFF'}")
        ...
        print(f" 📌 Reason      : {confirmed_reason}")
        print("------------------------------------------------")
